...
class Md2Html {
  ...
  initMarkdown() {
    ...
    this.md.renderer.rules.fence = (tokens, idx, options, env, self) => {
      ...
      if (token.info === 'mermaid') {
        ...
        const { cacheKey: diagramCacheKey, svgCacheDir } = require('../common/diagramCache')
        const code = token.content.trim()
        const mermaidConfig = env._mermaidConfig || loadMermaidConfig(null)
        const specRoot = env._specRootPath || ''
        const [result] = renderMermaidCached([code], mermaidConfig, specRoot)
        if (result && result.svg && specRoot) {
          const key = diagramCacheKey(code, mermaidConfig)
          const svgPath = require('path').join(svgCacheDir(specRoot), `${key}.svg`)
          const { width, height } = getSvgDimensions(result.svg)
          return `<div class="mermaid-figure"${attrs}><img src="${svgPath}" width="${width}" height="${height}"></div>`
        }
        return `<pre class="mermaid"${attrs}>${token.content}</pre>`
      }
      if (token.info === 'mscgen') {
        const { renderMscgenCached, getSvgDimensions } = require('../md2docx/handlers/mscgenHandler')
        ...
        const specRoot = env._specRootPath || ''
        const [result] = renderMscgenCached([code], configJson, specRoot)
        if (result && result.svg && specRoot) {
          const key = diagramCacheKey(code, configJson)
          const svgPath = require('path').join(svgCacheDir(specRoot), `${key}.svg`)
          const { width, height } = getSvgDimensions(result.svg)
          return `<div class="mscgen-figure"${attrs}><img src="${svgPath}" width="${width}" height="${height}"></div>`
        }
    ...
    // env._baseDir (tracked per-file via <!-- FILE: --> comments) and calls
    // resolveImageUri to produce the final URI (e.g. webview URI).
    // Runs regardless of forPreview so that both baseline and current renders
    // in change tracking produce identical URIs for unchanged images.
    const defaultImage = this.md.renderer.rules.image || function(tokens, idx, options, env, self) {
      return self.renderToken(tokens, idx, options)
    }
    this.md.renderer.rules.image = (tokens, idx, options, env, self) => {
      if (this.resolveImageUri) {
        const token = tokens[idx]
        ...
        if (src && !src.startsWith('http://') && !src.startsWith('https://') && !src.startsWith('data:')) {
          ...
          if (fs.existsSync(imagePath)) {
            token.attrSet('src', this.resolveImageUri(imagePath))
          }
        }
      }
   ...
   */
  renderBody(content, forPreview, baseDir = null, filePath = null, specRootPath = null, frontPageData = null, crCoverPageData = null) {
    if (!this.md) this.initMarkdown()

    const env = { _baseDir: baseDir, _mscgenConfig: this.mscgenConfig, _mermaidConfig: this.mermaidConfig, _specRootPath: specRootPath !== null ? specRootPath : this.specRootPath }
    ...
    for (let i = 0; i < tokens.length; i++) {
      if (tokens[i].type === 'paragraph_open') {
        ...
        if (kind === PARA.TOC) {
          ...
          let historyHtml = ''
          if (effectiveSpecRoot) {
            const { renderCRHistoryTableHTML } = require('../md2docx/crHistoryRenderer')
            historyHtml = renderCRHistoryTableHTML(effectiveSpecRoot)
          }
    ...
    // Both are rendered on the fly from their JSON data.
    if (crCoverPageData) {
      const { renderCRCoverPageHTML } = require('./crCoverPageRenderer')
      const crHtml = renderCRCoverPageHTML(crCoverPageData)
      htmlBody = crHtml + htmlBody
    } else if (frontPageData) {
      const { buildFrontPageHtml } = require('./frontPage')
      htmlBody = buildFrontPageHtml(frontPageData) + htmlBody
    }

    // Remove FILE comments from preview output (no longer needed after rendering)
    if (forPreview) {
      htmlBody = htmlBody.replace(/<!-- FILE: [^>]+ -->/g, '')
    }

    // Resolve absolute file paths in <img src> tags (e.g. cached diagram SVGs)
    // through resolveImageUri for preview (converts to vscode-resource URIs).
    // Skipped for export — exportHtmlFromDirectory handles image copying separately.
    if (forPreview && this.resolveImageUri) {
      htmlBody = htmlBody.replace(/<img([^>]*?)src="([^"]+)"([^>]*?)>/g, (match, before, src, after) => {
        ...
        if (path.isAbsolute(src) && fs.existsSync(src)) {
          return `<img${before}src="${this.resolveImageUri(src)}"${after}>`
        }
        return match
      })
    }
