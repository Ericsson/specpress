...
const imsizePlugin = require('markdown-it-imsize')
...
const { buildJsonTableIndices, classifyParagraph, extractText, injectAllSectionNumbers, insertBreakAfterColon, parseTocRange, PARA } = require('../common/specProcessor')
...
class Md2Html {
  ...
  initMarkdown() {
    ...
    this.md.renderer.rules.fence = (tokens, idx, options, env, self) => {
      ...
      if (token.info === 'asn') {
        ...
        if ((env._forPreview || env._annotateSourceLines) && token.attrGet && token.attrGet('data-source-line') !== null) {
   ...
   * @param {boolean} forPreview - If true, removes FILE comment markers from output.
   ...
   * @param {boolean} [annotateSourceLines] - If true, adds data-source-line/data-source-file
   *   attributes for scroll sync. Defaults to forPreview when omitted.
   * @returns {string} Rendered HTML body content.
   */
  renderBody(content, forPreview, baseDir = null, filePath = null, specRootPath = null, frontPageData = null, crCoverPageData = null, resolveUris, fileResolver, annotateSourceLines) {
    ...
    const doAnnotate = annotateSourceLines !== undefined ? annotateSourceLines : forPreview
    if (doAnnotate) {
...
${extraHeadContent}
   ...
   * @param {object} [options]
   * @param {object|null} [options.frontPageData]
   * @param {object|null} [options.crCoverPageData]
   * @returns {{ fileCount: number, imageCount: number, message: string }}
   */
  exportHtmlFromDirectory(inputDir, outputDir, options = {}) {
    const { exportHtml } = require('./exportHtml')
    return exportHtml({
      inputPaths: [path.resolve(inputDir)],
      outputPath: path.join(path.resolve(outputDir), 'index.html'),
      specRoot: this.specRootPath || path.resolve(inputDir),
      css: this.css,
      mermaidConfig: this.mermaidConfig,
      mscgenConfig: this.mscgenConfig,
      frontPageData: options.frontPageData || null,
      crCoverPageData: options.crCoverPageData || null,
    })
  }
}
