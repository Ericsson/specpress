...
class Md2Html {
  ...
  constructor(options = {}) {
    ...
    this.frontPageHtml = options.frontPageHtml || ''
  }
  ...
  initMarkdown() {
    ...
    this.md.use(emoji)
    const katexEngine = {
      renderToString: (latex, opts) => katex.renderToString(preprocessLatex(latex, opts && opts.displayMode), { ...opts, strict: false })
    }
   ...
   * @param {boolean} [includeFrontPage=false] - Whether to include front page.
   * @param {object|null} [crCoverPageData=null] - CR cover page data (takes precedence over frontPageHtml).
   * @returns {string} Rendered HTML body content.
   */
  renderBody(content, forPreview, baseDir = null, filePath = null, specRootPath = null, includeFrontPage = false, crCoverPageData = null) {
    ...
    if (includeFrontPage) {
      // CR cover page takes precedence over standard front page
      if (crCoverPageData) {
      ...
      } else if (this.frontPageHtml) {
        htmlBody = this.frontPageHtml + htmlBody
      }
    }
...
${extraHeadContent}
   ...
   * @param {boolean} [includeFrontPage=false] - Whether to include front page.
   * @param {object|null} [crCoverPageData=null] - CR cover page data.
   * @returns {string} Complete HTML document string with embedded scripts.
   */
  renderMarkdown(content, baseDir = null, filePath = null, specRootPath = null, includeFrontPage = false, crCoverPageData = null) {
    const htmlBody = this.renderBody(content, true, baseDir, filePath, specRootPath, includeFrontPage, crCoverPageData)
   ...
   * @returns {string} Complete HTML document string.
   */
  renderMarkdownForExport(content, specRootPath = null, includeFrontPage = false, crCoverPageData = null) {
    const htmlBody = this.renderBody(content, false, null, null, specRootPath, includeFrontPage, crCoverPageData)
