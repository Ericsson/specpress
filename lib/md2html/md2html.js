...
class Md2Html {
  ...
  initMarkdown() {
    ...
    this.md.renderer.rules.image = (tokens, idx, options, env, self) => {
      if (env._resolveUris && this.resolveImageUri) {
        const token = tokens[idx]
   ...
   * @param {boolean} [opts.sourceAnnotations=false] - Add data-source-line/file attributes for scroll sync.
  ...
  renderBody(content, forPreview, baseDir = null, filePath = null, specRootPath = null, frontPageData = null, crCoverPageData = null) {
    ...
    // URI resolution: apply resolveImageUri only for preview (when set and not exporting)
    const resolveUris = forPreview && !!this.resolveImageUri

    const env = { _baseDir: baseDir, _mscgenConfig: this.mscgenConfig, _mermaidConfig: this.mermaidConfig, _specRootPath: specRootPath !== null ? specRootPath : this.specRootPath, _resolveUris: resolveUris }
    ...
    // Resolve absolute file paths in <img src> tags (cached diagram SVGs)
