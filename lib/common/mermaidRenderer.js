...
const MERMAID_CDN_ESM = 'https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.esm.min.mjs'
...
function buildMermaidPageScript(codes, config) {
  return [
    `import mermaid from '${MERMAID_CDN_ESM}';`,
...
/**
 * Synchronously checks if the mermaid CDN ESM bundle is reachable.
 * Used by tests to skip rendering tests when there is no network access.
 * @returns {boolean}
 */
function canFetchMermaid() {
  try {
    require('child_process').execFileSync(process.execPath, [
      '-e',
      `const https=require('https');const req=https.request('${MERMAID_CDN_ESM}',{method:'HEAD'},r=>{process.exit(r.statusCode===200?0:1)});req.on('error',()=>process.exit(1));req.end()`
    ], { timeout: 5000 })
    return true
  } catch (e) {
    return false
