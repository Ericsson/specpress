...
function mergeViaLibreOffice(baseDocx, revisions, outputPath, options = {}) {
  ...
  // Resolve symlinks (on Linux, /usr/bin/soffice -> /usr/lib/libreoffice/program/soffice)
  const resolvedSoffice = fs.realpathSync(soffice)
  const programDir = path.dirname(resolvedSoffice)
  const pythonPath = findLibreOfficePython(resolvedSoffice)

  if (!pythonPath) {
    return Promise.reject(new Error(
      'LibreOffice bundled Python not found. Expected at: ' +
  ...
  // Use LibreOffice's bundled Python which has UNO bindings.
  // The script starts its own headless LibreOffice listener and connects via pipe.
  const cmd = pythonPath
  const args = [scriptPath, baseDocx, rev.docxPath, outputPath, rev.authorName, soffice]
  const fundamentalFile = process.platform === 'win32' ? 'fundamental.ini' : 'fundamentalrc'
  const fundamentalPath = path.join(programDir, fundamentalFile)
  const urePrefix = process.platform === 'win32' ? 'file:///' : 'file://'
  const env = Object.assign({}, process.env, {
    URE_BOOTSTRAP: urePrefix + fundamentalPath.replace(/\\/g, '/'),
    UNO_PATH: programDir,
    PATH: programDir + (process.platform === 'win32' ? ';' : ':') + (process.env.PATH || '')
  })
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.stderr.on('data', (data) => {
      ...
      errorOutput += text
      if (options.onProgress) options.onProgress(text.trim())
    })
