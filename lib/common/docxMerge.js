...
function mergeViaLibreOffice(baseDocx, revisions, outputPath, options = {}) {
  ...
  // LibreOffice's bundled Python has UNO bindings available.
  // On Linux, system python3 works if python3-uno is installed.
  const pythonPath = findLibreOfficePython(soffice)
  const programDir = path.dirname(soffice)

  let cmd, args, env
  if (pythonPath) {
    // Use LibreOffice's Python directly with UNO environment
    cmd = pythonPath
    args = [scriptPath, baseDocx, rev.docxPath, outputPath, rev.authorName]
    env = Object.assign({}, process.env, {
      URE_BOOTSTRAP: `file://${programDir.replace(/\\/g, '/')}/fundamentalrc`,
      UNO_PATH: programDir
    })
  } else {
    // Fallback: invoke via soffice --python (LibreOffice 7.4+)
    cmd = soffice
    args = ['--headless', '--invisible', '--python', scriptPath,
