...
/**
 * Checks whether the token at idx is a paragraph containing a {ChangeHistory} placeholder.
 */
function isChangeHistoryPlaceholder(tokens, idx) {
  if (tokens[idx].type !== 'paragraph_open') return false
  const inline = tokens[idx + 1]
  if (!inline || inline.type !== 'inline') return false
  return /^\{ChangeHistory\}$/.test(extractText(inline).trim())
}
...
function classifyParagraph(tokens, idx, jsonTableIndices) {
  ...
  if (/^\{ChangeHistory\}$/.test(text.trim())) return PARA.CR_HISTORY
  ...
  if (/^(\*\*)?Table/i.test(text)) {
    ...
    if (followIdx < tokens.length && (
      ...
      isChangeHistoryPlaceholder(tokens, followIdx)
