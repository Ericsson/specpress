...
function renderCached(opts) {
  const { codes, config, specRoot, prefix, cachePng, renderFn } = opts
  ...
  if (uncached.length > 0) {
    ...
    for (let j = 0; j < uncached.length; j++) {
      ...
      if (svg) fs.writeFileSync(path.join(cacheDir, `${prefix}${uncached[j].key}.svg`), svg)
      if (cachePng && png) fs.writeFileSync(path.join(cacheDir, `${prefix}${uncached[j].key}.png`), png)
    }
  }
...
/**
 * Cached rendering pipeline for diagrams. Supports async renderFn.
 *
 * Reads cached SVG+PNG from disk; renders uncached ones via renderFn and
...
function cleanupDiagramCache(specRoot, opts = {}) {
  ...
  for (const h of collectDiagramHashes(specRoot, MSCGEN_FENCE_RE, (raw) => applyMscgenPreamble(raw, mscgenPreamble), mscgenConfigJson)) {
    allLiveHashes.add(h)
  }
