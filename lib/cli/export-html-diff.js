...
const { getRepoRoot, extractFilesFromCommit, makeCachedTextReader } = require('../common/gitHelpers')
...
function getContent(commit, inputPaths, specRoot) {
  if (commit === 'local') {
    ...
    return concatenateFiles(files, undefined, specRoot)
  }

  const repoRoot = getRepoRoot(inputPaths[0])
  const cache = extractFilesFromCommit(repoRoot, commit, inputPaths)
  ...
  return concatenateFiles(files, textReader, specRoot)
}
...
function run() {
  ...
  try {
    ...
    const baselineContent = getContent(opts.base, inputPaths, specRoot)

    console.log(`Loading revision from "${opts.revision}"...`)
    const currentContent = getContent(opts.revision, inputPaths, specRoot)
