...
async function generateDocx(commit, inputPaths, specRoot, mermaidConfig, tempDir, frontPageData, crCoverPageData) {
  ...
  if (commit === 'local') {
    ...
    // Collect file paths from cache (sorted)
  ...
  if (crCoverPageData && fs.existsSync(crCoverPageData)) {
    try {
      const { loadCRCoverPageData } = require('../common/crCoverPageLoader')
      const result = loadCRCoverPageData(crCoverPageData)
      if (result.valid) crData = result.data
    } catch (e) { /* skip CR cover page on error */ }
  }
...
async function run() {
  ...
  // Default authors to revision identifiers if not provided
  if (!opts.authors.length) {
    opts.authors = opts.revisions.map((r, i) => r === 'local' ? 'Local changes' : r.slice(0, 8))
  }
