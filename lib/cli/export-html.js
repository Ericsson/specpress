 ...
 *     [--no-embed-images]
 *
 * By default images are embedded as base64 data URIs (self-contained file).
 * Use --no-embed-images to copy images to a media/ directory instead.
...
function parseArgs(argv) {
  ...
  while (i < args.length) {
    ...
    if (a === '--embed-images') { opts.embedImages = true; i++; continue }
    if (a === '--no-embed-images') { opts.embedImages = false; i++; continue }
...
function run() {
  ...
  if (errors.length) {
    console.error('Usage: node lib/cli/export-html.js <inputDir> <outputFile> [--spec-root <dir>] [--commit <ref>] [--css <file>] [--mermaid-config <file>] [--mscgen-config <file>] [--front-page-data <file>] [--cr-cover-page-data <file>] [--no-embed-images]')
  ...
  try {
    const result = exportHtml({
      ...
      embedImages: opts.embedImages !== undefined ? opts.embedImages : true,
      frontPageData,
      crCoverPageData,
    })
