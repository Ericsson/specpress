...
class MarkdownToDocxConverter {
  ...
  handleMermaidSvg(svg, elements, tokens, idx, ctx) {
    ...
    // Word ignores dominant-baseline, alignment-baseline, and dy attributes.
    // For dominant-baseline="central": shift y down by ~5px (half cap height).
    ...
    // For dy="1em": bake the em offset into y and remove dy (Word ignores dy).
    // Also add half font-size for dominant-baseline="middle" which Word ignores.
   ...
   * @param {object|null} [frontPage=null] - Front page data (standard front page).
   * @param {object} [options={}] - Additional options.
   * @param {object} [options.crCoverPageData=null] - CR cover page data (takes precedence over frontPage).
   */
  async convert(markdownPath, outputPath, baseDir, frontPage, options = {}) {
    ...
    const sections = []
    if (frontPage || options.crCoverPageData) {
