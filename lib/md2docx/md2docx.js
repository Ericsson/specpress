...
const { renderMermaidBatch, renderMermaidCached, getSvgDimensions, MERMAID_FENCE_RE } = require('./handlers/mermaidHandler')
const { renderMscgenBatch, renderMscgenCached, MSCGEN_FENCE_RE } = require('./handlers/mscgenHandler')
const { collectDiagramHashes, cleanupCacheFiles } = require('../common/diagramCache')
...
class MarkdownToDocxConverter {
  /**
   * @param {string|null} mermaidConfig - Mermaid configuration JSON string, or null.
   ...
   * @param {Function|null} [fileResolver=null] - Optional function `(absolutePath) => Buffer|string`
   *   for reading external files (images, JsonTable JSON). When null, reads from the local filesystem.
   *   Use this to resolve files from git commits or other non-filesystem sources.
   * @param {object} [options] - Additional options.
   * @param {string|null} [options.mscgenConfig] - MSC-Gen configuration JSON string.
   */
  constructor(mermaidConfig, specRootPath, mermaidRenderer, fileResolver, options) {
    this.mermaidConfig = mermaidConfig || null
    this.specRootPath = specRootPath || ''
    this.mermaidRenderer = mermaidRenderer || null
    this.fileResolver = fileResolver || null
    this.mscgenConfig = (options && options.mscgenConfig) || null
    this.updateFields = !(options && options.updateFields === false)
  ...
  readFile(filePath, encoding) {
    if (this.fileResolver) {
      const content = this.fileResolver(filePath)
  ...
  async beforeWalk(tokens, ctx) {
    ...
    const mermaidSvgs = mermaidEntries.length > 0
      ? (this.mermaidRenderer
          ...
          : renderMermaidCached(mermaidEntries.map(e => e.code), mermaidConfig, this.specRootPath || ''))
      : []
    ...
    for (let i = 0; i < tokens.length; i++) {
      if (tokens[i].type === 'fence' && tokens[i].info === 'mscgen') {
        ...
        mscgenEntries.push({ idx: i, code })
      }
    }
    const mscgenSvgs = mscgenEntries.length > 0
      ? renderMscgenCached(
          mscgenEntries.map(e => e.code),
          mscgenConfigJson,
          this.specRootPath || ''
        )
      : []
    ...
    // Unified diagram cache cleanup: collect live hashes for ALL diagram types,
    // then remove orphan cache files in one pass.
    if (this.specRootPath) {
      const allLiveHashes = new Set()
      // Mermaid live hashes
      for (const h of collectDiagramHashes(this.specRootPath, MERMAID_FENCE_RE, (raw) => raw, mermaidConfig)) {
        allLiveHashes.add(h)
      }
      // MSC-Gen live hashes
      const mscgenPreamble = parseMscgenPreamble(mscgenConfigJson)
      for (const h of collectDiagramHashes(this.specRootPath, MSCGEN_FENCE_RE, (raw) => applyMscgenPreamble(raw, mscgenPreamble), mscgenConfigJson)) {
        allLiveHashes.add(h)
      }
      cleanupCacheFiles(this.specRootPath, '', ['.svg', '.png'], allLiveHashes)
    }
   ...
   * @param {string} [baseDir] - Base directory for resolving relative paths.
   * @param {Object} [frontPage] - Front page sections from buildFrontPageDocx.
   * @returns {Promise<void>}
   */
  /**
   ...
   * @param {string|null} [baseDir=null] - Base directory for resolving relative paths.
   * @param {object|null} [frontPageData=null] - Front page JSON data (rendered on the fly).
   * @param {object} [options={}] - Additional options.
   * @param {object} [options.crCoverPageData=null] - CR cover page JSON data (takes precedence over frontPageData).
   */
  async convert(markdownPath, outputPath, baseDir, frontPageData, options = {}) {
