...
const { getImageDimensions, scaleToFit, MAX_IMAGE_WIDTH } = require('../common/imageUtils')
    ...
    for (const child of inlineToken.children) {
      if (child.type === 'image') {
        ...
        try {
          ...
          const explicitWidth = child.attrGet('width')
          const explicitHeight = child.attrGet('height')
          const dims = getImageDimensions(imageBuffer, ext)
          let scaled
          if (explicitWidth) {
            let w
            if (String(explicitWidth).endsWith('%')) {
              w = Math.round(MAX_IMAGE_WIDTH * parseFloat(explicitWidth) / 100)
            } else {
              w = parseInt(explicitWidth)
            }
            scaled = explicitHeight
              ? { width: w, height: parseInt(explicitHeight) }
              : { width: w, height: Math.round(dims.height * (w / dims.width)) }
          } else {
            scaled = scaleToFit(dims.width, dims.height)
          }
          this.docElements.push(new Paragraph({
