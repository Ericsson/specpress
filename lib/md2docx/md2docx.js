...
const { renderMermaidCached, renderMscgenCached } = require('../common/diagramRenderers')
...
class Md2Docx {
  ...
  async beforeWalk(tokens, ctx) {
    ...
    const mscgenSvgs = mscgenEntries.length > 0
      ? await renderMscgenCached(
          mscgenEntries.map(e => e.code),
          mscgenEntries[0].cacheConfig,
          this.specRootPath || '',
          (codes) => require('../common/mscgenRenderer').renderMscgenBatch(
            codes.map((code, j) => ({ code, type: mscgenEntries[j].type }))
          )
        )
      : []
