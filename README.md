...
- **Images** are embedded in DOCX with their native aspect ratio preserved. Large images are scaled to fit the page width; small images are not upscaled beyond 125 DPI to avoid pixelation. An explicit display size can be specified using a GitLab-style `{width= height=}` attribute block placed **directly after** the image link (no space in between):
  ...
  - The explicit size is respected in both HTML preview/export and DOCX export.
  - Because the image link itself stays a plain `![alt](path)`, editors such as VS Code keep recognizing the path (rename/refactor, go-to-file and broken-link detection continue to work).
